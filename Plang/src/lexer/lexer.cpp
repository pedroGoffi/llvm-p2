...
#define DEF_INTERN(x) x##_keyword = uniqueString.makeIntern(#x)
	DEF_INTERN(new);
	DEF_INTERN(if);
	DEF_INTERN(elif);
	DEF_INTERN(else);
...
static Token makeToken(Lexer* lexer, const char* fc, const char* lc, TokenKind kind, long i64) {
	return {
		...
		.lastChar = lc,
		.kind = kind,
		.name = uniqueString.makeIntern(fc, lc),
...
void SrcLocation::newContext(const char* filename){
	this->name			= uniqueString.makeIntern(filename);
	this->lineNumber	= 0;
	this->lineOffset	= 0;
}
...
const char* UniqueString::makeIntern(const char* begin, const char* end){
	size_t len = end - begin;
	for (InternString*& f : *this) {
		if (f->size == len and std::strncmp(begin, f->data, len) == 0) return f->data;
	}

	const char* str = strf("%.*s", len, begin);
	InternString* is = new InternString{
		.data = str,
...
const char* UniqueString::makeIntern(const char* data) {
	return this->makeIntern(data, data + std::strlen(data));
}
...
char Lexer::charToScape(char c){
	switch (c) {
		case 't': return '\t';			// \t	Insert a tab in the text at this point.
		case 'b': return '\b';			// \b	Insert a backspace in the text at this point.
		case 'n': return '\n';			// \n	Insert a newline in the text at this point.
		case 'r': return '\r';			// \r	Insert a carriage return in the text at this point.
		case 'f': return '\f';			// \f	Insert a form feed in the text at this point.
		case '\'': return '\'';			// \'	Insert a single quote character in the text at this point.
		case '"': return '\"';			// \"	Insert a double quote character in the text at this point.
		case '\\': return '\\';			// \\  
	}
	return -1;
}
long Lexer::scanInteger() {
...
double Lexer::scanFloat() {
	...
	const char* name = uniqueString.makeIntern(begin, this->stream.parsePoint);	
...
const char* Lexer::scanString() {
	...
	buff += '\0';
	this->advanceChar();
	return uniqueString.makeIntern(buff.begin()._Ptr, buff.end()._Ptr);
}
...
void make_test(const char* data){
	...
	const char* intern_1 = uniqueString.makeIntern("new");
	const char* intern_2 = uniqueString.makeIntern("new");
