...
class UniqueString: public std::vector<InternString*> {
public:
    const char* makeIntern(const char* begin, const char* end);
	const char* makeIntern(const char* data);
};
...
#define fatal_error(...) (error(__VA_ARGS__), system("pause"), exit(1))
